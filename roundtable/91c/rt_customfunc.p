...
FUNCTION fnRtbGetLintFile RETURNS CHARACTER
  ( INPUT prObject AS RECID)  FORWARD.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER rtb_ver    FOR rtb.rtb_ver.
  DEFINE BUFFER rtb_object FOR rtb.rtb_object.
  ...
  FIND rtb_ver WHERE rtb_ver.task-num   = Grtb-task-num 
                 AND rtb_ver.obj-status = "W":U 
                 AND rtb_ver.OBJECT     = cTemp NO-LOCK NO-ERROR.
  /* If the object exists, still doesnt mean you cant edit it.
     Could be file is not in RTB */
  IF NOT AVAILABLE rtb_ver THEN DO:
    FIND rtb_object WHERE rtb_object.wspace-id = grtb-wspace-id 
                      AND rtb_object.OBJECT    = cTemp NO-LOCK NO-ERROR.
    lCanEdit = NOT AVAILABLE rtb_object.  /* Not in RTB ? allow edit */
  END.
  ELSE lCanEdit = TRUE.
...
------------------------------------------------------------------------------*/
  ...
  FIND brtb_object
    WHERE RECID(brtb_object) = prObject
    NO-LOCK
    NO-ERROR.

  IF AVAILABLE brtb_object THEN
    RETURN 
      fnRtbGet4glFile( fnRtbGetSourceName( RECID(brtb_object),
                                     fnRtbGetRootPath( INPUT brtb_object.OBJECT,
...
------------------------------------------------------------------------------*/
    ...
    inifile = SEARCH(SUBSTITUTE("prolint/custom/rtb-check-in/ws-&1.ini":U, grtb-wspace-id)).
...
------------------------------------------------------------------------------*/
  ...
  THEN
    RETURN ENTRY(1, grtb-wspath).
  ELSE
    RETURN ENTRY(1, grtb-taskpath).
