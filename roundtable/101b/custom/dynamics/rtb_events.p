...
------------------------------------------------------------------------------*/
   ...
   /* Start : Roundtable Tabletop Title enhancement */
   DEFINE VARIABLE hWindowHandle               AS HANDLE      NO-UNDO.      
   /* End : Roundtable Tabletop Title enhancement */
  ...
  ELSE DO:
    ...
    */  
      ...
      ASSIGN cErrorMessage = "":U.
      
      /* Start : Roundtable Tabletop Title enhancement */
      /* Reset the title of the main RTB window  */
      PUBLISH "evRtbGetDeskHandle":U (OUTPUT hRTBDeskHandle). 
      IF VALID-HANDLE(hRTBDeskHandle) THEN
      DO:
         ASSIGN 
             hWindowHandle = hRTBDeskHandle:CURRENT-WINDOW
             hWindowHandle:TITLE = "Roundtable":U.              
      END.
      /* End : Roundtable Tabletop Title enhancement */
      ...
      /* First walk the widget tree and get the handles */
        ...
        ASSIGN hLoopHandle = SESSION:FIRST-PROCEDURE.
      ...
      IF  pEvent  NE "changeWorkspaceBefore" THEN 
      DO:
        IF NOT VALID-HANDLE(hRtbEvent) THEN 
        DO:
          IF SEARCH("rtb/prc/ryrtbevntp.p":U) NE ? OR 
             SEARCH("rtb/prc/ryrtbevntp.r":U) NE ? THEN
            RUN rtb/prc/ryrtbevntp.p PERSISTENT SET hRtbEvent.
        END.
      END. 
      
      IF  pEvent   = "changeWorkspaceBefore":U AND 
          ENTRY(1,pOther) NE "":U THEN /* only if previous workspace valid - kill running plips */ 
      DO:        
        ...
        PUBLISH "evRtbGetDeskHandle":U (OUTPUT hRTBDeskHandle). 
        RUN rtb/prc/afrtbmenup.p (INPUT hRTBDeskHandle).
      END.
      
      IF pEvent = "changeWorkspace":U THEN 
      DO:
        FIND FIRST rtb.rtb_wspace WHERE rtb_wspace.wspace-id = pOther NO-LOCK NO-ERROR. 
        ...
        /* Add the Progress Dynamics menu to the tabletop */
             /* Set Grtb-proc-handle  */  
         PUBLISH "evRtbGetDeskHandle":U (OUTPUT hRTBDeskHandle). 
         RUN rtb/prc/afrtbmenup.p (INPUT hRTBDeskHandle) NO-ERROR.  
