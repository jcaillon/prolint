...
PROCEDURE InitializeOutputhandler :
    ...
    IF handlers > "" THEN DO i=1 TO NUM-ENTRIES(handlers) :
...
PROCEDURE LintOneSourceFile :
    ...
    ELSE DO:
        ...
        IF ErrorMessage="":U THEN DO:
            FOR EACH tt_rules NO-LOCK WHERE tt_rules.sourcefile > "" :
...
PROCEDURE PreAnalyze :
    ...
    IF NeedProparse AND HasProparse AND p-ErrorMessage="":U THEN DO:
        ...
        ASSIGN lParserResult = parserParse(p-Sourcefile) NO-ERROR.
        IF ERROR-STATUS:ERROR THEN
            p-ErrorMessage = ERROR-STATUS:GET-MESSAGE(1).
        ELSE IF NOT lParserResult THEN
