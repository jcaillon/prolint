...
PROCEDURE InitializePlugins :
    ...
    REPEAT:
        IMPORT progname.
        IF progname MATCHES "*~~.p":U THEN DO:
            CREATE tt_filters.
    ...
    FILE-INFORMATION:FILE-NAME = "prolint/custom/filters":U.
    fulldir = FILE-INFORMATION:FULL-PATHNAME.
    IF fulldir <> ? THEN DO:
        INPUT FROM OS-DIR ( fulldir ).
        REPEAT:
            ...
            IF progname MATCHES "*~~.p":U THEN DO:
                CREATE tt_filters.
                RUN VALUE( "prolint/custom/filters/":U + progname )
                    PERSISTENT SET tt_filters.hpFilter ( pProfileDirectory ).
                tt_filters.filtername = LC( SUBSTRING( progname, 1, LENGTH( progname ) - 2 ) ).
            END.
        END.
...
PROCEDURE GetFilterResult :
    ...
    ELSE
        FOR EACH tt_filters :
            RUN GetFilterResult IN tt_filters.hpFilter  (
                pCompilationUnit,
                pFullSource,
                pRelativeSource,
                pLineNumber,
