/* -----------------------------------------------------------------------------
   file    :  prolint/rules/bufdbfunc.p
...
DEFINE TEMP-TABLE tt_temptables NO-UNDO
   ...
   FOR EACH tt_procedure WHERE tt_procedure.proctype = "FUNCTION" AND tt_procedure.prototype=false :

       EMPTY TEMP-TABLE tt_definedbuffers.
       EMPTY TEMP-TABLE tt_reportedtables.
       
       RUN AddFunctionParameters (tt_procedure.startnode).
...
procedure AddFunctionParameters :
    ...
    /*  We are only interested in buffer parameter nodes. */
    if nodetype = "Parameter_list":U THEN DO:
        paramtype = parserNodeFirstChild(child,child).
        do while paramtype <> "":
            if paramType = "BUFFER":U THEN DO:
               nodetype = parserNodeFirstChild(child, paramID).
               DO WHILE nodetype<>"" :
                  CASE nodetype :
                     WHEN "ID":U THEN buffername = parserGetNodeText(paramID).
                     WHEN "RECORD_NAME":U THEN DO:
                                                   parserAttrSet(paramID, pragma_number ,1).
                                                   tablename = parserGetNodeText(paramID).
                                                   CREATE tt_definedbuffers.
                                                   ASSIGN tt_definedbuffers.buffername = buffername
                                                          tt_definedbuffers.tablename = tablename.
                                               END.
                  END CASE.
                  nodetype = parserNodeNextSibling(paramID, paramID).
               END.
            end.
            paramtype = parserNodeNextSibling(child,child).
        end.
    end.
